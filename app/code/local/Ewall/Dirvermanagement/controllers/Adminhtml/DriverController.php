...
class Ewall_Dirvermanagement_Adminhtml_DriverController extends Mage_Adminhtml_Controller_Action
{
	...
	public function assigndriverAction(){
		...
		try{
			if($order->getId()){
				...
				{
					$driver_name  = Mage::getModel('admin/user')->load($driver_id)->getUsername();
					$comment =  'Order Assigned To Driver " '.$driver_name .' "';
				}
				else
				{
					$comment =  'Order Driver UnAssigned';
				}
	...
	public function assignstatusAction(){
		...
		try{
			...
			if($order->getId()){
		        ...
		        // Cancel an order
		        if(/* 
				// TO ALLOW direct change orderstatus to returned Islam Elgarhy
				$model->getOrderStatus() == 'returned' || 
		        ...
		        if($order->getOrderStatus() == 'dispatched'){
		        	/*if(Mage::getStoreConfig('customconfig_options/section_smsconfig/active')) {
			            if($telephone = $order->getBillingAddress()->getTelephone()){
			                ...
			                Mage::log($driverid , null, 'mylog3.log');
			                $fields = array(
			                    'username' => Mage::getStoreConfig('customconfig_options/section_smsconfig/apiuname'),
			                    'password' => Mage::getStoreConfig('customconfig_options/section_smsconfig/apipwd'),
			                    'customerid' => '361',
			                    'sendertext' => 'HiNet GCC',
			                    'messagebody' => str_replace('{{order link}}', $order_url . ' Driver Phone ' . $driverdata[mobile] , Mage::getStoreConfig('customconfig_options/section_smsconfig/dispatchmessage')),
	...
	public function updateStatusAction(){
		...
		$driverid = $this->getRequest()->getParams();
	    if ($fieldId) {
	        $model = Mage::getModel('sales/order')->load($fieldId);
	        ...
	        if($model->getOrderStatus() == 'dispatched'){
	        	/*if(Mage::getStoreConfig('customconfig_options/section_smsconfig/active')) {
		            ...
		            if($telephone = $model->getBillingAddress()->getTelephone()){
		                $url = Mage::getStoreConfig('customconfig_options/section_smsconfig/smsconfigurl');
						$driver_id = $model->getDriverId();
						// $driver_model  = Mage::getModel("dirvermanagement/driver")->load($driver_id);
						// $driverdata = Mage::getModel('dirvermanagement/driver')->getCollection()
						// 			->addFieldToFilter('userid', array('eq' => $driver_id));
						$driverdata = Mage::getModel("dirvermanagement/driver")->load($driver_id,'userid')->getData();
						// $dri = $driverdata->getData();
		                $order_urls = 'sales/guest/view/order_id/'.Mage::helper('core')->urlEncode($model->getIncrementId());        
		                $order_url = Mage::getUrl($order_urls, array('_secure' => true));
		                Mage::log($order_url , null, 'mylog3.log');
						Mage::log($driverdata[mobile] , null, 'mylog3.log');
						$tst = str_replace('{{order link}}', $order_url . ' Driver Phone ' . $driverdata[mobile] , Mage::getStoreConfig('customconfig_options/section_smsconfig/dispatchmessage'));
		                Mage::log($tst , null, 'mylog3.log');
		                $fields = array(
		                    'username' => Mage::getStoreConfig('customconfig_options/section_smsconfig/apiuname'),
		                    'password' => Mage::getStoreConfig('customconfig_options/section_smsconfig/apipwd'),
		                    'customerid' => '361',
