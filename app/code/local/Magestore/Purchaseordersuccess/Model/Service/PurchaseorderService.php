    ...
    public function confirm($purchaseOrder)
    {
        ...
        if ($this->savePurchaseOrderStatus($purchaseOrder, PurchaseorderStatus::STATUS_PROCESSING))
            Mage::getSingleton('adminhtml/session')
                ->addSuccess(Mage::helper('purchaseordersuccess')->__('Confirm purchase order successfully.'));
        else
            Mage::getSingleton('adminhtml/session')
                ->addError(Mage::helper('purchaseordersuccess')->__('Cannot confirm purchase order.'));;
    }
    ...
    public function addProductsToPurchaseOrder($purchaseOrder, $supplier, $data)
    {
        ...
        if (count($data)) {
            ...
            foreach ($supplierProducts as $supplierProduct) {
                ...
                $cost = $supplierProduct->getCost() * $purchaseOrder->getCurrencyRate();
                $tax = isset($data[$productId][PurchaseorderItem::TAX]) ?
                    $data[$productId][PurchaseorderItem::TAX] : $supplierProduct->getTax();
                ...
                $productData[$supplierProduct->getProductId()] = array(
                    ...
                    PurchaseorderItem::PRODUCT_ID => $supplierProduct->getProductId(),
                    PurchaseorderItem::PRODUCT_SKU => $supplierProduct->getProductSku(),
                    PurchaseorderItem::PRODUCT_NAME => $supplierProduct->getProductName(),
                    PurchaseorderItem::PRODUCT_SUPPLIER_SKU => $supplierProduct->getProductSupplierSku(),
