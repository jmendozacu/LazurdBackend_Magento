...
class Magestore_Webpos_Model_Api2_Customer_Rest_Admin_V1 extends Magestore_Webpos_Model_Api2_Abstract
{
    ...
    public function getCustomerList()
    {
        ...
        $customerCollection
            ...
            ->addExpressionAttributeToSelect('telephone', 'IFNULL({{phone}},"N/A")', array('phone'));
           
        $customerCollection
            ->getSelect()
           // ->join( array('cev' => 'customer_entity_varchar'), 'cev.entity_id=e.entity_id', array('telephone' => 'value'))
            ->where('website_id != 0 ')
            //->columns('IFNULL(cev.value,"N/A") AS telephone')
  ...
  Mage::log($customerCollection->getSelect()->__toString() ,null,'mylog55.log');
        ...
        foreach ($customerCollection as $customerModel) {
            ...
            foreach ($customerAddresses as $address) {
                ...
                $receivername = $address->getData('receiver_name');
                $address->setData('receiver_name', $receivername);
            ...
            if(!$customerModel->getData('dob')){
                $customerModel->setData('dob','');
            }
            $customers[] = $customerModel->getData();
        }

        if ($pageNumber <= ($numberOfCustomer/$pageSize+1)) {
    ...
    public function loadCustomer($customerId = false)
    {
        ...
        foreach ($customerAddresses as $address) {
            ...
            $isgift = $address->getData('is_gift');
            $address->setData('is_gift', $isgift);

            $addressesData[] = $address->getData();
        }
        //echo "<pre>";print_r($addressesData);die;
        $customerModel->setData('addresses', $addressesData);
            ...
            if ( isset($customerInformation['id']) && !is_numeric($customerInformation['id'])) {
                ...
                foreach($addresses as $addressData) {
                   ...
                   // $addressData['region'] = $addressRegion['region'];
					...
					$regionArray=array();
					$regionArray = array('region' => $addressData['region']['region'],'region_id' => $addressData['region']['region_id']);
					$addressData['region'] = $regionArray;
					// echo "regionArray"; print_r($regionArray);
                ...
                $customer->setData($customerInformation);
                ...
                if ($postAddress !== null) {
                    ...
                    foreach ($postAddress as $address) {
                       ...
                       // $address['region'] = $address['region']['region'];
						...
						$address['receiver_name'] = ($address['receiver_name']) ? $address['receiver_name'] : '';
                        //$address['is_gift'] = ($address['is_gift'] == 1) ? '1' : '0';;

						$address['is_gift'] = $address['is_gift'] ;
                        //$addressModel = Mage::getModel('customer/address')->load($address['entity_id']);
                        if(!$address['telephone']){
                            $address['telephone'] = $customerInformation['phone'];
                            $address['phone'] = $customerInformation['phone'];
                        }
