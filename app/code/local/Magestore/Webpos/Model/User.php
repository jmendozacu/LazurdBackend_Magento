...
class Magestore_Webpos_Model_User extends Mage_Core_Model_Abstract
{
    ...
    protected function _beforeSave()
    {
        $data = array(
            ...
            'till_ids' => ($this->getTillIds())?implode(',', $this->getTillIds()):Magestore_Webpos_Model_Till::VALUE_ALL_TILL,
        );
        if ($this->getId() > 0) {
    ...
    public function loadByUsername($username,$store_id)
    {
        $info = $this->getCollection()->addFieldToFilter('username', $username)->addFieldToFilter('store_id', array('finset'=>$store_id));
        if ($id = $info->getFirstItem()->getId())
            $this->load($id);
        return $this;
    }

    public function authenticate($login, $password,$store_id)
    {
        $this->loadByUsername($login,$store_id);
...
/*Edit by 24122017 Islam ELgarhy*/
    public function toOptionArray() {
        ...
        if ($userCollection->getSize() > 1) {
            $options = array('' => '---Select Supervisor User---');
        }
