...
class Magestore_Webpos_Helper_Order extends Mage_Core_Helper_Abstract {
    ...
    public function getAllOrderInfo($order) {
        ...
        $order->setData('items_info_buy', ['items' => $itemInfoBuy]);
        ...
        if ($shippingAddress)
            $order->setData('extension_attributes', ['shipping_assignments'=> [['shipping' => ['address' => $shippingAddress->getData()]]]]);
        return $order;
    }
    ...
    public function getOrderData($order) {
        ...
        $orderData['shipping_arrival_comments'] = $order->getData('shipping_arrival_comments');
        $orderData['shipping_arrival_date'] = $order->getData('shipping_arrival_date');
        $orderData['shipping_delivery_date'] = $order->getData('shipping_delivery_date');
    
         // OrderStatus&State&Status Islam Elgarhy
        $orderData['order_status'] = $order->getData('order_status');
       // OrderStatus&State&Status Islam Elgarhy


       $store = Mage::getModel('core/store')->load($order->getData('store_id'));
       $name = $store->getName();
       $orderData['store_nameOnly'] =   $name;
       
      // Islam Elgarhy Print As Magento 
       $customer = Mage::getModel('customer/customer')->load($orderData['customer_id']);
       $orderData['customer_phone_original'] = $customer->getData("phone");
       return $orderData;
    }
