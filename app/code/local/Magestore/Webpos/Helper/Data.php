...
class Magestore_Webpos_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getWebposConfig($session = false) {
        ...
        $config = array(
            ...
            'backOrders' => Mage::helper('webpos/config')->isEnablePoleDisplay(),
        );

        // Edit by Jacob
        if (Mage::helper('core')->isModuleEnabled('Cmsmart_Deliverydate')){
            $delivery = Mage::getModel('deliverydate/deliverydate')->getCollection(); 
            $config['deliverydate'] = $delivery->getData();
        }
        // End
