...
class Mage_Reports_Model_Resource_Report_Collection_Abstract extends Mage_Core_Model_Resource_Db_Collection_Abstract
{
    ...
    protected function _applyStoresFilterToSelect(Zend_Db_Select $select)
    {
        ...
        $storeIds[0] = ($storeIds[0] == '') ? 0 : $storeIds[0];
       	...
       	//webpos reports fix - Rohit Starts
       	$action = Mage::app()->getFrontController()->getAction()->getFullActionName();
       	
       	if ((strpos($action, 'adminhtml_report_sales_staff') !== false) || (strpos($action, 'adminhtml_report_sales_location') !== false) ||(strpos($action, 'adminhtml_report_sales_payment') !== false) ) {
       		if ($nullCheck) {
       			$select->where('sales_flat_order.store_id IN(?) OR store_id IS NULL', $storeIds);
       		} else {
       			$select->where('sales_flat_order.store_id IN(?)', $storeIds);
       		}
       		
       		return $this;
       	} 
       	//webpos reports fix - Rohit Ends
