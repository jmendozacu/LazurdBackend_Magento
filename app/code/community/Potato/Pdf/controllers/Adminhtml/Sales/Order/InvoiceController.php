...
class Potato_Pdf_Adminhtml_Sales_Order_InvoiceController extends Mage_Adminhtml_Sales_Order_InvoiceController
{
    public function printAction()
    {
        $invoice = Mage::getModel('sales/order_invoice')->load($this->getRequest()->getParam('invoice_id', null));
        if (!Potato_Pdf_Helper_Adminhtml::isAdminInvoicePrintAllowed($invoice->getStoreId())) {
            return parent::printAction();
        }
        $template = Mage::getModel('po_pdf/template')->load(
            Potato_Pdf_Helper_Config::getInvoiceAdminTemplate($invoice->getStoreId()))
        ;

        //prepare html
        $html = $template->getProcessedTemplate(Mage::helper('po_pdf/sales')->getInvoiceVariables($invoice), true);

        return $this->_convertAndSendPdf(array($html), Potato_Pdf_Helper_Data::getFileName('invoice-'), $invoice->getStoreId());
    }
