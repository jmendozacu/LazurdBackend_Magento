...
define(
    ...
    function ($, modelAbstract, orderRest, orderIndexedDb, orderCollection, staffHelper) {
        ...
        return modelAbstract.extend({
            ...
            isCanceled: function () {
                return (this.data.state === 'canceled' || this.data.order_status === 'canceled');
            },
            ...
            canCancel: function () {
                ...
                /*if (this.data.status === 'approved' && this.data.webpos_order_payments.length == 1 && this.data.webpos_order_payments[0].method == "codforpos")
                return true;*/
               
                 if (this.data.status === 'pending' && this.data.webpos_order_payments.length == 1 && this.data.webpos_order_payments[0].method == "codforpos")
                    return true;
...
// wait Islam Elgarhy for Edit
                if (this.data.state === 'complete' || this.data.state === 'closed' || this.data.order_status === 'canceled' ) {
                    return false;
                }
                // Edit Order 7/2018
                /*
                else if (this.data.webpos_order_payments.length == 1 && this.data.webpos_order_payments[0].method == "codforpos") {
                    return true;
                }
                */
               // Edit Order 7/2018
