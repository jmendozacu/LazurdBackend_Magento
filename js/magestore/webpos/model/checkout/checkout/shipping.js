        ...
        var ShippingModel = {
            ...
            initialize: function () {
                ...
                Event.observer('load_shipping_online_after', function (event, data) {
                    if (data && data.items) {
                        debugger;
                        var invoiceTotal = CheckoutModel.getTotalDue();
                       
                        ko.utils.arrayForEach(data.items, function(item, index) {
                            if(item.code == 'flatrate_flatrate'){
                               if(item.free_shipping_subtotal <= invoiceTotal){
                                   item.price = "0.0000";
                               }
                            }
                        });                        
            ...
            getShippingAddress: function () {
                return CheckoutModel.getOrderParams();
            },
            /**
