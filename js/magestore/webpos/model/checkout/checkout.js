...
define(
    ...
    function ($, ko, CartModel, DiscountModel/*,EditModel*/, HelperPrice, OrderHelper, Staff, DateTime, Event, Totals, Helper, Alert, OrderFactory, CheckoutResource, CreditCard, DataManager, AddNoti, Tills, restAbstract) {
        "use strict";
        ...
        var CheckoutModel = {
            ...
            /*Edit by 24122017 Islam ELgarhy*/
            cartDiscountCause: ko.pureComputed(function () {
                if (DiscountModel.cartDiscountAmount() > 0 && !DiscountModel.cartDiscountCause()) {
                    return 'Custom Discount';
                }
                return DiscountModel.cartDiscountCause();
            }),

            cartDiscountBy: function () {
                return Staff.getStaffId();
            },

            //getDiscountCausePopup :function(){
            //    if(EditModel.getData('custom_price_amount') > 0)
            //        return EditModel.getdiscountCause()
            //    return "";
            //},
            /*Edit by 24122017 Islam ELgarhy*/
                    ...
                    request.done(function (response) {
                        if (response && response.data) {
                            ...
                            if (data.increment_id) {
                                OrderFactory.get().delete(data.increment_id).done(function () {
            ...
            saveBillingAddress: function (data) {
                if (data.id == 0) {
                    ...
                    if (typeof data.phone != 'undefined') {
                        newAddress.telephone = data.phone;
                    }
            ...
            placeOrderOnlineDefault: function () {
                var self = this;
                var deferred = $.Deferred();
                if (CartModel.hasErrors() && CartModel.errorMessages()) {
                    ...
                    params.actions = {
                        ...
                        need_approval: self.needApproval(),
                        need_watting: self.needWaitting(),
                        shipping_arrival_date: self.shippingArrivalDate(),
                        shipping_arrival_comments: self.shippingArrivalComments(),
                        shipping_arrival_time_slot: self.shippingArrivalTimeSlot()
                    };
                    params.integration = [];
                    ...
                    deferred.done(function (response) {
                        if (response.status && response.data) {
                            if (response.data.increment_id) {
                                var message = Helper.__('Order has been created successfully ') + "#" + response.data.increment_id;
                                AddNoti(message, true, "success", Helper.__('Message'));
                            }
                            self.createOrderResult(response.data);
                            Event.dispatch('webpos_place_order_online_after', { data: response.data });
                            localStorage.removeItem("OrderIsEdit")
                            localStorage.removeItem("OrderIsEditId");
                            // Can't Save After edit some order
            ...
            prepareDeleteOrder: function () {
                ...
                if (CartModel.hasErrors() && CartModel.errorMessages()) {
                    ...
                    var params = CartModel.getQuoteInitParams();
                    var self = this;
                    var apiUrl = '/webpos/order/delete';
                    var deferred = $.Deferred();

                    var orderId = localStorage.getItem("OrderIsEditId");
                    var orderQuoteId = params.quote_id;
                    ...
                    deferred.done(function (data) {
                        return self.placeOrderOnlineDefault();
                    });
                    deferred.fail(function (data) { });
                }
            },
            placeOrderOnline: function () {
                if (localStorage.getItem("OrderIsEdit") == "true") {
                    this.prepareDeleteOrder();
                }
                else
                    return this.placeOrderOnlineDefault();
