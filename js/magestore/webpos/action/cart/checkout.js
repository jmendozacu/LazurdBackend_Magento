...
define(
    ...
    function ($, CheckoutModel, CartModel, DiscountModel, SelectCustomer, OrderFactory, Totals, CustomerFactory, Helper) {
        ...
        return function (orderData) {
            ...
            CheckoutModel.shippingAddress(params.shipping_address);
            CartModel.shippingAddress(params.shipping_address);

            if(params.shipping_address.is_gift === '1')
            {
                CartModel.giftAddres(params.shipping_address);
            }
            ...
            if ($('#checkout').length > 0) {
                ...
                if(params.shipping_address.is_gift === '1')
                {
                    Helper.dispatchEvent('update_gift_work', '');
                }
            }
