...
define(
    ...
    function ($, ko, SelectCustomer, AddNoti, CheckoutOrder, PriceHelper, Item, CheckoutModel,
        CartModel, ProductFactory, CustomerFactory, restAbstract, priceHelper, ItemInterface, Event) {
        'use strict';
                    ...
                    if (items && items.length > 0) {
                                ...
                                ko.utils.arrayForEach(response.items, function (product, index) {
                                    ko.utils.arrayForEach(items, function (item, index) {
                                        if (item.id == product.entity_id) {
                                            ...
                                            infoBuy.options_label = [];
                                            infoBuy.options = []

                                            $.each(item.optionsWithTitle, function (i, v) {
                                                if(v.value.optionTitle != null && v.value.optionTitle != undefined){
                                                infoBuy.options.push(v.value);
                                                }
                                            });
                                            infoBuy.options_label = Product.getOptionsLabelInfoRequest(infoBuy.options);
                                            CartModel.addProduct(infoBuy);
                                            Product.resetTempAddData();
                                        }
                                    });
                                });
                              Event.dispatch('apply_from_edit', '');
                            } else {
                        ...
                        if (orderData.customer_id) {
                            var customerData = {
                                ...
                                full_name: (orderData.customer_firstname == null ? '' : orderData.customer_firstname) + " " + (orderData.customer_lastname == null  ? '' : orderData.customer_lastname),
                                lastname: orderData.customer_lastname,
                                group_id: orderData.customer_group_id
                            }; 

                            var customerDeferred = CustomerFactory.get().setMode('online').load(orderData.customer_id);
                            ...
                            customerDeferred.done(function (data) {
                                ...
                                data.full_name = customerData.full_name;
