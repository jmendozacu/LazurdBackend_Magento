...
define([
    ...
    return Component.extend({
        ...
        loadLocalConfig: function() {           
            ...
            //var selectedId = Helper.getLocalConfig('backorderConfig');
            this.backorderConfig = localConfig.get('backorderConfig'); 
        },
        ...
        prepareAddToCart: function (product,event) {
            debugger;
            var self = this;
            /* Edit By Ryan 31122017*/
           if ( ( (product.isBackorders == 0 && product.useConfigBackorders == 0 ) ||  ( product.useConfigBackorders == 1 && self.backorderConfig  == 0) ) && product.available_qty < 1  ) {
                var productModel = ProductFactory.get();
                productModel.setData(product);
                Alert({
                    priority:'danger',
                    title: self.__('Warning'),
                    message: self.__('This product is currently out of stock and Can\'t be Backorder ')
                });
                return false;
                $("#search-header-product").val("");
            ...
            if (product.options == 1) {
                ...
                if (product.super_group && product.super_group.length > 0) {
                    ...
                    if (product.isBackorders == 2 || ( product.useConfigBackorders == 1 && self.backorderConfig  == 2) ) {
                        Alert({
                            priority:'warning',
                            title: self.__('Warning'),
                            message: self.__('The product \'' + product.name +  '\' Will be Backorder')
                        });
                    }
