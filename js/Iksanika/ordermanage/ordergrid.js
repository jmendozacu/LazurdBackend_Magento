...
var varienGridIksanikaOrdermanage = Class.create(varienGrid);



varienGridIksanikaOrdermanage.prototype.initialize = function(containerId, url, pageVar, sortVar, dirVar, filterVar){
...
varienGridIksanikaOrdermanage.prototype.initGrid = function(){
...
varienGridIksanikaOrdermanage.prototype.rowMouseOver = function(event)
{
...
varienGridIksanikaOrdermanage.prototype.rowMouseMove = function(event)
{
};

varienGridIksanikaOrdermanage.prototype.addMultiSelectedRow = function(event, rowElement) 
{
...
varienGridIksanikaOrdermanage.prototype.findCheckbox = function(evt) 
{
...
varienGridIksanikaOrdermanage.prototype.removeMultiSelectedRow = function(event, rowElement) {
...
varienGridIksanikaOrdermanage.prototype.rowMouseUp = function(event){
    if(this.doMultiSelect) 
    {
        enableHighlighting();
        this.doMultiSelect = false;
...
varienGridIksanikaOrdermanage.prototype.rowMouseDown = function(event)
{
    if(event.ctrlKey) {
...
varienGridIksanikaOrdermanage.prototype.rowMouseClick = function(event)
{
    if(this.doMultiSelect) 
    {
        this.doMultiSelect = false;
...
// Multiselect 
varienGridIksanikaOrdermanage.prototype.onMultiSelect = function(events) 
{
    for(rowid in events) 
    {
...
var varienGridMassactionIksanikaOrdermanage = Class.create(varienGridMassaction);

varienGridMassactionIksanikaOrdermanage.prototype.initialize = function (containerId, grid, checkedValues, formFieldNameInternal, formFieldName) 
{
    // MAGE -- begin
...
// Multiselect 
varienGridMassactionIksanikaOrdermanage.prototype.onMultiSelect = function(events) 
{
...
varienGridMassactionIksanikaOrdermanage.prototype.apply = function() 
{
        ...
        this.getCheckedValues().split(",").each(function(item){
            ...
            if(multiFields != null) 
            {
                for (var i=0; i<multiFields.length; i++) 
                {
                    ...
                    if($j('#editData #order'+item+' [name='+multiFields[i]+']').is("textarea"))
                    {
                        itemValue = $j('#editData #order'+item+' [name='+multiFields[i]+']').val();
                    }else
                        itemValue = $j('#editData #order'+item+' [name='+multiFields[i]+']').val();
...
varienGridMassactionIksanikaOrdermanage.prototype.walkSelectedRows = function(walkFunction, warningLimit) 
{
...
varienGridIksanikaOrdermanage.prototype.doFilter = function()
{
...
varienGridIksanikaOrdermanage.prototype.resetFilter = function()
{
    if($$('input#advancedGridSearchQry')[0] != undefined) 
    {
        this.addVarToUrl('q', '')
